...
Horn.prototype.render = function (enlarge) {
...
$(document).click(function (event) {
  const zoomID = 'enlarge';
  const $select = $(event.target);
  for (let i = 0; i < Horn.allHorns.length; i++) {
    if ($select[0].src === Horn.allHorns[i].image) {
      $('.main-content').empty();
      $('.main-content').append(Horn.allHorns[i].render(zoomID));
    }
  }
  if ($($select).parent()[0].id === zoomID) {
    $('.main-content').empty();
